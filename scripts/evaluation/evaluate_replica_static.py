...
from src.evaluation.baselines.adapters import adapt_conceptgraphs, adapt_dualmap
...
def main() -> int:
    ...
    parser.add_argument("--baseline", choices=("conceptgraphs", "dualmap"), required=True)
    ...
    required_by_baseline = {
        "dualmap": ("map_dir", "class_names", "layout", "runtime_csv"),
        "conceptgraphs": ("map_file", "detection_log", "mapping_log", "gpu_dmon"),
    }
    ...
    artifact = (
        _load_dualmap(args, manifest)
        if args.baseline == "dualmap"
        else _load_conceptgraphs(args, semantic_vocabulary)
    )
