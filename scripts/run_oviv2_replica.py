...
from src.oviv2.runner_config import (  # noqa: E402
...
def _runtime_config(config: dict[str, Any]) -> Oviv2RuntimeConfig:
    return runtime_config_from_json(config)


def _semantic_fusion_config(
    config: dict[str, Any],
) -> SemanticFusionConfig | None:
    return semantic_fusion_config_from_json(config)


def _structure_config(config: dict[str, Any], *, voxel_size_m: float) -> DepthStructureConfig:
    return structure_config_from_json(config, voxel_size_m=voxel_size_m)
