...
confidence parser. Instrument only observations in the unchanged evaluate_matches using sys.settrace;
...
- [x] Bind current files from the two FP32/T1/evaluation receipts; hash only consumed assets once.
...
- [x] Add fusion_attribution.py and test_static_t1_attribution.py. Fail first on missing functions;
...
- [x] Add run_static_t1_attribution.py and frozen config. Save one mask bank/IoU cache/run, all
...
- [x] Add evaluator trace module and diagnostic runner. Deterministic fixtures cover duplicate
...
- [x] Finish fixed geometry bridge with historical emitted registry/scores; report eligibility
...
- [x] Generate all four machine-readable tables: paired performance; intervention differences and
...
- [x] Assess repeat agreement by GT-object outcomes, never raw query ID. Decide whether exactly
  one prediction-only repair is justified; otherwise NO_REPAIR_JUSTIFIED. No threshold grid.
- [x] Final spec sections 1–10 audit, evidence-based decision, measured costs, exact executed
...
Final execution and requirement audit: T1_ATTRIBUTION_RESULTS.md and T1_ATTRIBUTION_AUDIT.md. Remote commit verification is recorded by the final Git operation, not inferred from this checklist.
