...
Rebuild the AAAI manuscript around one scientific claim: an online open-vocabulary semantic map should represent the current scene state rather than an irreversible accumulation of past observations. OVIV2 is the proposed method; the main benchmark suite is the evidence spine that tests this claim through controlled scene changes, stale-surface removal, revealed-background recovery, causal ablations, and online efficiency.
...
- **Method consequence:** independent TSDF geometry, dense semantic evidence, persistent entity posteriors, signed visibility, reversible ownership, and background reclaim.
...
3. Dynamic scenes expose three coupled failures: stale object surfaces remain in the map, occlusion is confused with absence, and newly revealed background is not recovered correctly.
...
5. OVIV2 separates these states and uses signed visibility evidence to make current ownership reversible and reclaim newly revealed surfaces.
...
| 3 | Does the current map remove stale surfaces and recover revealed space? | Common current-map metrics on TESSE-CD | Signed visibility and reversible ownership improve current-state map quality | Current mIoU, ghost rate, background F@5cm, recovery frames |
| 4 | Which mechanisms cause the gains? | Cumulative causal ablation | Negative evidence, ownership release, and background reclaim address distinct dynamic failures | Static mIoU guardrail, change F1, ghost rate, background F@5cm, recovery frames |
...
5. background reclaim restores surfaces exposed by relocation or removal;
...
4. **Causal ablation.** Add signed visibility, reversible ownership, and background reclaim in the same order as the method. Measure change F1, ghost rate, background F@5cm, and recovery frames; static mIoU is only a guardrail.
