...
class OutputProfile:
    ...
    benchmark_profile: str = "debug_overlay"
    headline_excludes_runtime_vis_debug_artifacts: bool = False
    runtime_vis_debug_images_enabled: bool = True
...
class Room0ExportResult:
    ...
    finalization_sec: float = 0.0
    largest_export_size_bytes: int = 0
...
def output_profile_to_dict(output_profile: OutputProfile) -> dict[str, Any]:
    benchmark_profile = str(output_profile.benchmark_profile)
    if output_profile.name == "fast_eval" and benchmark_profile not in {"fast", "debug_overlay"}:
        benchmark_profile = "fast"
    if output_profile.name == "fast_eval" and benchmark_profile == "debug_overlay":
        benchmark_profile = "fast"
    return {
        ...
        "headline_excludes_runtime_vis_debug_artifacts": bool(
            output_profile.headline_excludes_runtime_vis_debug_artifacts or benchmark_profile == "fast"
        ),
        "runtime_vis_debug_images_enabled": bool(
            output_profile.runtime_vis_debug_images_enabled and benchmark_profile != "fast"
        ),
    }
...
def output_profile_from_checkpoint(raw_profile: Any) -> OutputProfile:
    ...
    raw_benchmark_profile = str(profile.get("benchmark_profile", "") or "").strip().lower()
    if raw_benchmark_profile not in {"fast", "debug_overlay"}:
        raw_benchmark_profile = "fast" if str(profile.get("name", "")) == "fast_eval" else "debug_overlay"
    return OutputProfile(
        ...
        benchmark_profile=raw_benchmark_profile,
        headline_excludes_runtime_vis_debug_artifacts=bool(
            profile.get("headline_excludes_runtime_vis_debug_artifacts", False)
        ),
        runtime_vis_debug_images_enabled=bool(profile.get("runtime_vis_debug_images_enabled", True)),
    )
...
def _normalize_benchmark_profile(args: argparse.Namespace) -> str:
    raw = str(getattr(args, "benchmark_profile", "") or "").strip().lower()
    if raw in {"fast", "debug_overlay"}:
        return raw
...
def _runtime_vis_debug_excluded_sec(benchmark_timing: dict[str, Any] | None) -> float:
    timing = dict(benchmark_timing or {})
    return _coerce_float(timing.get("runtime_vis_debug_overlay_sec_excluded_from_headline", 0.0))


def _headline_online_mapping_sec(benchmark_timing: dict[str, Any] | None) -> float | None:
    if benchmark_timing is None or benchmark_timing.get("online_mapping_sec") is None:
        return None
    online_sec = _coerce_float(benchmark_timing.get("online_mapping_sec"))
    if bool(benchmark_timing.get("headline_excludes_runtime_vis_debug_artifacts", False)):
        return max(0.0, online_sec - _runtime_vis_debug_excluded_sec(benchmark_timing))
    return online_sec


def audit_checkpoint_payload_sizes(payload: dict[str, Any]) -> dict[str, Any]:
    """Estimate checkpoint size contribution by top-level payload key."""
    key_sizes: dict[str, Any] = {}
    for key in sorted(payload):
        try:
            size_bytes = len(pickle.dumps({key: payload[key]}, protocol=pickle.HIGHEST_PROTOCOL))
            key_sizes[str(key)] = {"size_bytes": int(size_bytes), "method": "pickle"}
        except Exception as exc:  # pragma: no cover - defensive for unusual runtime objects.
            key_sizes[str(key)] = {
                "size_bytes": len(repr(payload.get(key, "")).encode("utf-8", errors="replace")),
                "method": "repr",
                "error": type(exc).__name__,
            }
    largest = sorted(
        (
            {"key": key, "size_bytes": int(value.get("size_bytes", 0)), "method": str(value.get("method", ""))}
            for key, value in key_sizes.items()
        ),
        key=lambda item: item["size_bytes"],
        reverse=True,
    )
    return {
        "key_count": int(len(key_sizes)),
        "top_level_key_sizes": key_sizes,
        "largest_keys": largest[:10],
    }


def _iter_object_debug_dicts(state: SystemState, key: str) -> list[dict[str, Any]]:
    values: list[dict[str, Any]] = []
    for obj in state.objects.values():
        raw = obj.debug.get(key, {})
        if isinstance(raw, dict):
            values.append(raw)
    return values


def _iter_local_pool_debug_dicts(state: SystemState) -> list[dict[str, Any]]:
    values: list[dict[str, Any]] = []
    for obj in state.objects.values():
        raw = obj.debug.get("_local_pcd_chunk_pool")
        if not isinstance(raw, dict):
            raw = obj.debug.get("local_geometry_pool")
        if isinstance(raw, dict):
            values.append(raw)
    return values


def build_benchmark_contract_payload(
    *,
    state: SystemState,
    frame_metrics: list[dict[str, Any]],
    online_mapping_sec: float | None = None,
    finalization_sec: float | None = None,
    eval_io_sec: float | None = None,
    mapping_state_size_bytes: int | None = None,
    largest_export_size_bytes: int | None = None,
) -> dict[str, Any]:
    local_pool_debugs = _iter_local_pool_debug_dicts(state)
    association_debugs = _iter_object_debug_dicts(state, "association_geometry")
    final_local_points = int(sum(len(obj.local_pcd) for obj in state.objects.values()))
    frame_local_points = [
        _coerce_int(metrics.get("local_memory_point_count_total", 0))
        for metrics in frame_metrics
        if isinstance(metrics, dict)
    ]
    max_local_points = max([final_local_points, *frame_local_points], default=final_local_points)
    pending_peaks = [
        _coerce_int(debug.get("pending_point_peak", debug.get("local_pcd_pending_point_peak", 0)))
        for debug in local_pool_debugs
    ]
    export_sizes: list[int] = []
    if largest_export_size_bytes is not None:
        export_sizes.append(_coerce_int(largest_export_size_bytes))

    raw_online_mapping_sec = _coerce_float(online_mapping_sec)
    return {
        "online_mapping_sec": raw_online_mapping_sec,
        "headline_online_mapping_sec": raw_online_mapping_sec,
        "finalization_sec": _coerce_float(finalization_sec),
        "eval_io_sec": _coerce_float(eval_io_sec),
        "end_to_end_sec": (
            _coerce_float(online_mapping_sec)
            + _coerce_float(finalization_sec)
            + _coerce_float(eval_io_sec)
        ),
        "final_local_memory_point_count_total": final_local_points,
        "max_local_memory_point_count_total": int(max_local_points),
        "local_pcd_pending_point_peak": int(max(pending_peaks, default=0)),
        "local_pcd_compaction_count": int(
            sum(_coerce_int(debug.get("compaction_count", debug.get("local_pcd_compaction_count", 0))) for debug in local_pool_debugs)
        ),
        "local_pcd_compaction_sec": float(
            sum(_coerce_float(debug.get("compaction_sec", debug.get("local_pcd_compaction_sec", 0.0))) for debug in local_pool_debugs)
        ),
        "association_geometry_sketch_update_count": int(
            sum(
                _coerce_int(
                    debug.get(
                        "association_geometry_sketch_update_count",
                        debug.get("sketch_update_count", debug.get("sketch_pending_update_count", 0)),
                    )
                )
                for debug in association_debugs
            )
        ),
        "association_geometry_sketch_new_key_count": int(
            sum(
                _coerce_int(
                    debug.get(
                        "association_geometry_sketch_new_key_count",
                        debug.get("sketch_new_key_count_total", debug.get("sketch_new_key_count", 0)),
                    )
                )
                for debug in association_debugs
            )
        ),
        "association_geometry_sketch_dropped_key_count": int(
            sum(
                _coerce_int(
                    debug.get(
                        "association_geometry_sketch_dropped_key_count",
                        debug.get("sketch_dropped_key_count_total", debug.get("sketch_dropped_key_count", 0)),
                    )
                )
                for debug in association_debugs
            )
        ),
        "association_geometry_sketch_materialize_count": int(
            sum(
                _coerce_int(
                    debug.get(
                        "association_geometry_sketch_materialize_count",
                        debug.get("sketch_materialize_count", int(bool(debug.get("materialized_from_sketch", False)))),
                    )
                )
                for debug in association_debugs
            )
        ),
        "association_geometry_sketch_update_sec": float(
            sum(_coerce_float(debug.get("association_geometry_sketch_update_sec", 0.0)) for debug in association_debugs)
        ),
        "association_geometry_sketch_materialize_sec": float(
            sum(
                _coerce_float(debug.get("association_geometry_sketch_materialize_sec", 0.0))
                for debug in association_debugs
            )
        ),
        "mapping_state_size_bytes": _coerce_int(mapping_state_size_bytes),
        "largest_export_size_bytes": int(max(export_sizes, default=0)),
    }
...
def build_output_profile(args: argparse.Namespace) -> OutputProfile:
    ...
    benchmark_profile = _normalize_benchmark_profile(args)
    if bool(getattr(args, "full_debug", False)):
        return OutputProfile(
            ...
            write_reports=True,
            benchmark_profile="debug_overlay",
            headline_excludes_runtime_vis_debug_artifacts=False,
            runtime_vis_debug_images_enabled=True,
        )
    ...
    if bool(getattr(args, "fast_eval", False)):
        return OutputProfile(
            ...
            runtime_vis_debug_images_enabled=False,
        )
    ...
    return OutputProfile(
        ...
        write_reports=True,
        benchmark_profile=benchmark_profile,
        headline_excludes_runtime_vis_debug_artifacts=benchmark_profile == "fast",
        runtime_vis_debug_images_enabled=benchmark_profile != "fast",
    )


def apply_runtime_profile_to_pipeline(pipeline: Any, output_profile: OutputProfile) -> None:
    config = dict(getattr(pipeline, "config", {}) or {})
    runtime_vis_cfg = dict(config.get("runtime_vis", {}) or {})
    runtime_vis_cfg["benchmark_profile"] = output_profile.benchmark_profile
    if output_profile.benchmark_profile == "fast":
        runtime_vis_cfg["debug_images_enabled"] = False
        runtime_vis_cfg.setdefault("debug_every", 0)
    else:
        runtime_vis_cfg["debug_images_enabled"] = bool(output_profile.runtime_vis_debug_images_enabled)
        runtime_vis_cfg.setdefault("debug_every", 10)
    config["runtime_vis"] = runtime_vis_cfg
    pipeline.config = config
    runtime_vis_module = getattr(pipeline, "runtime_vis", None)
    if runtime_vis_module is not None:
        runtime_vis_module.config = runtime_vis_cfg
...
def build_prefetch_pipeline(config_path: Path, args: argparse.Namespace) -> Pipeline:
    ...
    apply_runtime_profile_to_pipeline(prefetch_pipeline, build_output_profile(args))
...
def export_room0_outputs(
    *,
    ...
    benchmark_timing: dict[str, Any] | None = None,
) -> Room0ExportResult:
    ensure_room0_run_directories(layout, output_profile)
    benchmark_audit_enabled = bool(output_profile.benchmark_audit_enabled)
    dense_surface_module = getattr(pipeline, "dense_surface", None)
    object_update_module = getattr(pipeline, "object_update", None)
    finalization_sec = _coerce_float((benchmark_timing or {}).get("finalization_sec", 0.0))
    if object_update_module is not None and hasattr(object_update_module, "flush_deferred_geometry"):
        flush_start = time.perf_counter()
        object_update_module.flush_deferred_geometry(state)
        finalization_sec += float(time.perf_counter() - flush_start)
    if dense_surface_module is None:
        config = dict(getattr(pipeline, "config", {}) or {})
        dense_surface_module = DenseSurfaceModule(config.get("dual_map", {}))
    if dense_surface_module is not None and hasattr(dense_surface_module, "refresh_all_for_export"):
        state = dense_surface_module.refresh_all_for_export(state, frame_id=max(int(frame_limit) - 1, 0))
    ...
    largest_export_size_bytes = max(
        (path.stat().st_size for root in (layout.exports_dir, layout.eval_dir) for path in root.glob("*.ply")),
        default=0,
    )
    ...
    if output_profile.write_reports:
        write_run_report(
            ...
            benchmark_timing={
                **dict(benchmark_timing or {}),
                "finalization_sec": float(finalization_sec),
                "largest_export_size_bytes": int(largest_export_size_bytes),
            },
        )
...
def main() -> None:
    ...
    print(f"benchmark_profile: {output_profile.benchmark_profile}")
...
def write_run_report(
    ...
    benchmark_contract = build_benchmark_contract_payload(
        state=state,
        frame_metrics=frame_metrics,
        online_mapping_sec=(benchmark_timing or {}).get("online_mapping_sec"),
        finalization_sec=(benchmark_timing or {}).get("finalization_sec"),
        eval_io_sec=(benchmark_timing or {}).get("eval_io_sec"),
        mapping_state_size_bytes=(benchmark_timing or {}).get("mapping_state_size_bytes"),
        largest_export_size_bytes=(benchmark_timing or {}).get("largest_export_size_bytes"),
    )
    raw_online_mapping_sec = _coerce_float((benchmark_timing or {}).get("online_mapping_sec"))
    default_headline_online_mapping_sec = _headline_online_mapping_sec(benchmark_timing)
    if default_headline_online_mapping_sec is None:
        default_headline_online_mapping_sec = benchmark_contract["online_mapping_sec"]
    headline_online_mapping_sec = _coerce_float(
        (benchmark_timing or {}).get("headline_online_mapping_sec"),
        default_headline_online_mapping_sec,
    )
    benchmark_profile = str(
        (benchmark_timing or {}).get("benchmark_profile", output_profile.benchmark_profile)
        or output_profile.benchmark_profile
    )
    checkpoint_size_audit = dict((benchmark_timing or {}).get("checkpoint_size_audit", {}) or {})
    checkpoint_largest_payload_keys = list(
        (benchmark_timing or {}).get(
            "checkpoint_largest_payload_keys",
            checkpoint_size_audit.get("largest_keys", []),
        )
        or []
    )
    ...
    lines = [
        ...
        f"- `online_mapping_sec`: `{benchmark_contract['online_mapping_sec']:.4f}`",
        f"- `headline_online_mapping_sec`: `{headline_online_mapping_sec:.4f}`",
        f"- `raw_online_mapping_sec`: `{raw_online_mapping_sec:.4f}`",
        f"- `runtime_vis_debug_overlay_sec_excluded_from_headline`: `{_runtime_vis_debug_excluded_sec(benchmark_timing):.4f}`",
        f"- `finalization_sec`: `{benchmark_contract['finalization_sec']:.4f}`",
        f"- `eval_io_sec`: `{benchmark_contract['eval_io_sec']:.4f}`",
        f"- `end_to_end_sec`: `{benchmark_contract['end_to_end_sec']:.4f}`",
        ...
        f"- `max_local_memory_point_count_total`: `{benchmark_contract['max_local_memory_point_count_total']}`",
        f"- `local_pcd_pending_point_peak`: `{benchmark_contract['local_pcd_pending_point_peak']}`",
        f"- `local_pcd_compaction_count`: `{benchmark_contract['local_pcd_compaction_count']}`",
        f"- `local_pcd_compaction_sec`: `{benchmark_contract['local_pcd_compaction_sec']:.4f}`",
        f"- `association_geometry_sketch_update_count`: `{benchmark_contract['association_geometry_sketch_update_count']}`",
        f"- `association_geometry_sketch_materialize_count`: `{benchmark_contract['association_geometry_sketch_materialize_count']}`",
        f"- `mapping_state_size_bytes`: `{benchmark_contract['mapping_state_size_bytes']}`",
        f"- `largest_export_size_bytes`: `{benchmark_contract['largest_export_size_bytes']}`",
    ...
    if checkpoint_largest_payload_keys:
        lines.append("")
        lines.append("## Checkpoint Size Audit")
        for item in checkpoint_largest_payload_keys[:10]:
            if not isinstance(item, dict):
                continue
            lines.append(
                f"- `{item.get('key', '')}`: `{int(item.get('size_bytes', 0))}` bytes "
                f"via `{item.get('method', '')}`"
            )
    lines.extend(
        [
            "",
    ...
    sidecar = {
        ...
        **benchmark_contract,
        "headline_online_mapping_sec": headline_online_mapping_sec,
        "raw_online_mapping_sec": raw_online_mapping_sec,
        "runtime_vis_debug_overlay_sec_excluded_from_headline": _runtime_vis_debug_excluded_sec(benchmark_timing),
        "checkpoint_size_audit": checkpoint_size_audit,
        "checkpoint_largest_payload_keys": checkpoint_largest_payload_keys,
