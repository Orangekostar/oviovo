...
def event(entity_id: int = 1, **changes: object) -> TemporalLifecycleEvent:
    values: dict[str, object] = {
        ...
        "timestamp_ns": 1_250_000_000,
        "entity_id": entity_id,
        "before_lifecycle": TemporalLifecycle.ACTIVE,
        "after_lifecycle": TemporalLifecycle.UNCERTAIN,
        "evidence_kind": TemporalEvidenceKind.VISIBLE_ABSENT,
        "geometry_epoch": 0,
        "readout_valid": False,
    }
    values.update(changes)
...
@pytest.mark.parametrize(
    "bad_event",
    [
        lambda: event(before_lifecycle="active"),
        lambda: event(after_lifecycle="uncertain"),
        lambda: event(evidence_kind="visible_absent"),
