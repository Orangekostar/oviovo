...
def test_sam2_backend_anchor_prompts_preserve_anchor_labels() -> None:
    class FakePredictor:
        def __init__(self) -> None:
        ...
        def predict(self, *, box, multimask_output, return_logits, normalize_coords):
            self.predicted_boxes.append(np.asarray(box, dtype=np.float32))
            mask = np.zeros((8, 8), dtype=bool)
            mask[2:6, 2:6] = True
            return mask[None, ...], np.array([0.91], dtype=np.float32), None

    backend = SAM2ProposalBackend()
    backend.image_predictor = FakePredictor()
    backend.anchor_prompt_enabled = True
    backend.anchor_prompt_min_mask_area = 1
    backend.anchor_prompt_min_anchor_box_coverage = 0.1
    backend.anchor_prompt_max_masks_per_anchor = 1
    backend.max_proposals = 50
    backend.confidence_threshold = 0.0
    backend.device = "cpu"

    rgb = np.zeros((8, 8, 3), dtype=np.uint8)
    depth = np.ones((8, 8), dtype=np.float32)
    anchors = [Anchor2D(7, np.array([1, 1, 7, 7], dtype=np.float32), "chair", 0.84)]

    proposals = backend.generate_proposals_for_anchors(rgb, depth, anchors)

    assert backend.image_predictor.set_image_calls == 1
    assert len(proposals) == 1
    proposal = proposals[0]
    assert proposal.backend_name == "sam2_anchor_prompt"
    assert proposal.metadata["source"] == "anchor_prompted_sam"
    assert proposal.metadata["proposal_source"] == "anchor_prompted_sam"
    assert proposal.metadata["anchor_id"] == 7
    assert proposal.metadata["anchor_class"] == "chair"
    assert proposal.metadata["anchor_class_name"] == "chair"
    assert proposal.metadata["anchor_confidence"] == pytest.approx(0.84)
    assert proposal.metadata["anchor_label_votes"] == {"chair": pytest.approx(0.84)}
    assert proposal.metadata["semantic_commit_allowed"] is True
    assert proposal.metadata["sam_score"] == pytest.approx(0.91)
    assert backend.last_generation_info["proposal_source"] == "anchor_prompted_sam"
    assert backend.last_generation_info["kept_mask_count"] == 1
    assert "sam2_anchor_prompts" in backend.last_generation_timings


def test_sam2_backend_anchor_prompts_empty_anchors_skip_full_frame_sam() -> None:
    class FailFullFrameMaskGenerator:
        def generate(self, image):
            raise AssertionError("full-frame SAM should not run for empty anchor prompts")

    class FakePredictor:
        def set_image(self, image):
            raise AssertionError("set_image should not run without anchors")

    backend = SAM2ProposalBackend()
    backend.mask_generator = FailFullFrameMaskGenerator()
    backend.image_predictor = FakePredictor()
    backend.anchor_prompt_enabled = True
    backend.device = "cpu"
    ...
    assert proposals == []
    assert backend.last_generation_info["proposal_source"] == "anchor_prompted_sam"
    assert backend.last_generation_info["prompted_anchor_count"] == 0
    assert "sam2_anchor_prompts" in backend.last_generation_timings


def test_sam2_backend_anchor_prompts_skip_invalid_boxes() -> None:
    class FakePredictor:
        def set_image(self, image):
            raise AssertionError("set_image should not run when every anchor box is invalid")

    backend = SAM2ProposalBackend()
    backend.image_predictor = FakePredictor()
    backend.anchor_prompt_enabled = True
    backend.device = "cpu"
    anchors = [Anchor2D(1, np.array([3, 3, 3, 5], dtype=np.float32), "table", 0.7)]
    ...
    assert proposals == []
    assert backend.last_generation_info["skipped_anchor_count"] == 1


def test_proposal_module_rewrites_anchor_prompt_source_ids_after_common_filtering(monkeypatch) -> None:
    class FakeAnchorPromptBackend(ProposalBackend):
        def initialize(self, config: dict) -> None:
            self.anchor_prompt_enabled = True

        def generate_proposals(self, rgb, depth, frame=None):
            raise AssertionError("full-frame generation should not run")
        ...
        def generate_proposals_for_anchors(self, rgb, depth, anchors, frame=None):
            ...
            return [
                Proposal2D(
                    ...
                    metadata={"source": "anchor_prompted_sam", "anchor_id": 1, "source_raw_proposal_ids": [10]},
                ),
                Proposal2D(
                    proposal_id=11,
                    mask=large,
                    ...
                    confidence=0.9,
                    backend_name="sam2_anchor_prompt",
                    metadata={"source": "anchor_prompted_sam", "anchor_id": 2, "source_raw_proposal_ids": [11]},
                ),
            ]

    monkeypatch.setattr(ProposalModule, "_create_backend", lambda self, config: FakeAnchorPromptBackend())
    module = ProposalModule({"backend": "sam2", "min_mask_area": 4, "sam2": {"anchor_prompt": {"enabled": True}}})
    proposals = module.process_for_anchors(
        np.zeros((8, 8, 3), dtype=np.uint8),
        np.ones((8, 8), dtype=np.float32),
        [Anchor2D(2, np.array([2, 2, 6, 6], dtype=np.float32), "chair", 0.9)],
    )

    assert len(proposals) == 1
    assert proposals[0].proposal_id == 0
    assert proposals[0].metadata["source_raw_proposal_id"] == 0
    assert proposals[0].metadata["source_raw_proposal_ids"] == [0]
