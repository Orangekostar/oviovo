...
> **Branch:** `feat/semantic-vote-optimization-v3`  
> **Version:** v3 — Semantic Vote + local_pcd + Anchor Guided SAM Fusion Optimized  
> **Date:** 2026-06-18

## v3 Optimization Summary

This branch contains three rounds of engineering optimization targeting pipeline bottlenecks, achieving **4.7× overall stage-time speedup** on 200f stride=10 experiments with **mIoU preserved** (0.475 vs 0.480 baseline).

| Optimization | Module | Speedup | mIoU Impact |
|---|---|---|---|
| `local_pcd_update` vectorization | `object_update.py` | **3.5×** (2.38s→0.68s/frame) | Equivalent |
| Candidate Evidence gate | `object_update.py` | N/A (quality) | +0.083 (0.397→0.480) |
| `_semantic_vote_overlap_metrics` vectorization | `object_anchor.py` | **6.4×** (3.16s→0.49s proposal_generation) | -0.005 |

**Key technique:** Replaced per-voxel Python loops and full-frame mask AND operations with bbox pre-filtering and numpy slice-based overlap computation — same pattern applied across three independent hotspots.

**Experiment reference:** `outputs/tmp_validation/_20260618_semantic_vote_opt_s10_200f/`

---

# OVIOVO — Online Open-Vocabulary Instance-First Dynamic Mapping (Original)
