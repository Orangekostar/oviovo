...
# ============================================================
# Candidate Evidence Grid (for NEW OBJECT detection)
# ============================================================

@dataclass
class CandidateEvidenceEntry:
    """Per-voxel candidate object evidence for background regions.

    Tracks unassociated observations that repeatedly appear
    in a location not yet owned by any object.
    """
    label_votes: Dict[str, float] = field(default_factory=dict)
    hit_count: int = 0
...
@dataclass
class CandidateEvidenceGrid:
    """Sparse grid tracking candidate evidence on background voxels.

    Separate from TSDF owner_support:
    - TSDF arbitrates ownership for existing objects
    - CandidateEvidenceGrid tracks signals that may become new objects
    """
    voxel_size: float = 0.05
    entries: Dict[Tuple[int, int, int], CandidateEvidenceEntry] = field(default_factory=dict)
...
class SystemState:
    ...
    candidate_evidence: CandidateEvidenceGrid = field(default_factory=CandidateEvidenceGrid)
