...
class ProposalModule:
    ...
    def _build_backend(self, config: Dict[str, Any]) -> ProposalBackend:
        ...
        try:
            ...
            if self.requested_backend_name in {"placeholder", "precomputed", "sam3_concept", "sedpp"}:
                raise
            logger.warning(
    ...
    def process(
        ...
        self.last_generation_timings = dict(getattr(self.backend, "last_generation_timings", {}) or {})
    ...
    def _finalize_proposals(self, proposals: List[Proposal2D]) -> List[Proposal2D]:
        ...
        for proposal_id, proposal in enumerate(proposals):
            ...
            if str(proposal.metadata.get("proposal_source", proposal.metadata.get("source", ""))) == "anchor_prompted_sam":
                proposal.metadata["source_raw_proposal_id"] = int(proposal_id)
                proposal.metadata["source_raw_proposal_ids"] = [int(proposal_id)]
