...
class AnchorGuidedSAMModule:
    ...
    def build_proposals(
        ...
        *,
        ...
        anchored_count = 0
        frame_h, frame_w = frame.depth.shape
        valid_sam_proposals = [
            proposal
        ...
        # Pre-compute proposal areas and bboxes for the hot loop
        ...
        for proposal in valid_sam_proposals:
            ...
            if int(proposal.area) >= self.proposal_min_area:
                prop_area[pid] = int(np.asarray(proposal.mask, dtype=bool).sum())
        ...
        for anchor in anchors:
            ...
            for proposal in valid_sam_proposals:
                ...
                relation = self._classify_relation_fast(
                    proposal, anchor, anchor_bbox, anchor_area,
                    prop_area[pid], prop_bbox[pid],
                )
        ...
        for anchor in anchors:
            ...
            if selected:
                proposal = self._build_anchored_proposal(anchor, selected, anchor_bbox, anchor_area)
            ...
            if self.include_anchor_box_fallbacks:
                proposal = self._build_anchor_box_fallback(anchor, anchor_bbox, frame_h, frame_w)
                output_proposals.append(proposal)
    ...
    def _build_anchored_proposal(
        ...
        coverage = self._aggregate_coverages_bbox(mask, anchor_bbox, anchor_area, relations)
    ...
    def _build_anchor_box_fallback(self, anchor: Anchor2D, anchor_bbox: np.ndarray, frame_h: int, frame_w: int) -> Proposal2D:
        ...
        return Proposal2D(
            proposal_id=int(anchor.anchor_id),
            mask=fb_mask,
            bbox_xyxy=self._mask_bbox(fb_mask),
            area=int(fb_mask.sum()),
            confidence=float(anchor.confidence),
            backend_name="anchor_guided_sam",
            metadata=metadata,
        )
    ...
    def _aggregate_coverages_bbox(
        ...
        ax1, ay1, ax2, ay2 = [int(math.floor(float(v))) for v in anchor_bbox]
        # Overlap = mask pixels inside anchor bbox
        overlap_area = int(mask[ay1:ay2, ax1:ax2].sum())
        proposal_area = int(mask.sum())
        anchor_area_val = max(anchor_area, 1.0)
        if proposal_area > 0:
            return {
                "proposal_anchor_coverage": float(overlap_area / proposal_area),
                "anchor_proposal_coverage": float(overlap_area / anchor_area_val),
