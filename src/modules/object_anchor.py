...
class ObjectAnchorModule:
    ...
    def _generate_semantic_vote_proposals(
        ...
        # Pre-compute area and bbox per proposal (hot loop)
        prop_area: dict[int, int] = {}
        prop_bbox: dict[int, np.ndarray] = {}
        for proposal in proposals:
            pid = int(proposal.proposal_id)
            prop_area[pid] = max(int(np.asarray(proposal.mask, dtype=bool).sum()), 1)
            prop_bbox[pid] = np.asarray(proposal.bbox_xyxy, dtype=np.float32)

        for proposal in proposals:
            pid = int(proposal.proposal_id)
            assignments, blocked_candidates = self._assign_semantic_vote_candidates(
                proposal, self.last_anchors,
                _prop_area=prop_area[pid],
                _prop_bbox=prop_bbox[pid],
            )
            if not assignments:
    ...
    def _bboxes_overlap(bbox_a: np.ndarray, bbox_b: np.ndarray) -> bool:
        """True if two xyxy bboxes have non-zero intersection area."""
    ...
    def _semantic_vote_overlap_metrics_fast(
        self,
        proposal: Proposal2D,
        anchor: Anchor2D,
        proposal_area: int,
        ...
        """Compute overlap metrics using bbox-bounded slice (no full-mask AND)."""
        ...
        if ix2 <= ix1 or iy2 <= iy1:
            overlap_area = 0
            clipped_mask = np.zeros_like(proposal_mask, dtype=bool)
        else:
            clipped_mask = proposal_mask[iy1:iy2, ix1:ix2]
            overlap_area = int(clipped_mask.sum())
        anchor_area_px = max(int((ax2 - ax1) * (ay2 - ay1)), 1)
        anchor_coverage = overlap_area / anchor_area_px
        proposal_coverage = overlap_area / max(proposal_area, 1)
        center = np.asarray(
    ...
    def _assign_semantic_vote_candidates(
        ...
        *,
        _prop_area: int | None = None,
        _prop_bbox: np.ndarray | None = None,
        ...
        proposal_bbox = np.asarray(proposal.bbox_xyxy, dtype=np.float32) if _prop_bbox is None else _prop_bbox
        proposal_area = max(int(np.asarray(proposal.mask, dtype=bool).sum()), 1) if _prop_area is None else _prop_area

        for anchor in anchors:
            anchor_box = np.asarray(anchor.bbox_xyxy, dtype=np.float32)
            # Bbox pre-filter: skip expensive mask AND if bboxes don't overlap
            if not self._bboxes_overlap(anchor_box, proposal_bbox):
                continue

            metrics = self._semantic_vote_overlap_metrics_fast(
                proposal, anchor, proposal_area, proposal_bbox
            )
    ...
    def _generate_weak_structure_overlap_proposals(
        ...
        *,
        ...
        for anchor in anchors:
            ...
            # Build full clipped mask from slice for the weak proposal
            ax1, ay1, ax2, ay2 = [float(v) for v in anchor_box]
            px1, py1, px2, py2 = [float(v) for v in proposal_bbox]
            ix1 = max(0, int(math.floor(max(ax1, px1))))
            iy1 = max(0, int(math.floor(max(ay1, py1))))
            ix2 = min(int(proposal_bbox[2]) + 1, int(math.ceil(min(ax2, px2))))
            iy2 = min(int(proposal_bbox[3]) + 1, int(math.ceil(min(ay2, py2))))
            pmsk = np.asarray(proposal.mask, dtype=bool)
            full_clip = np.zeros_like(pmsk, dtype=bool)
            if ix2 > ix1 and iy2 > iy1:
                full_clip[iy1:iy2, ix1:ix2] = pmsk[iy1:iy2, ix1:ix2]

            weak = self._weak_structure_overlap_proposal(
                proposal, anchor,
                full_clip, metrics, weak_index=len(weak_proposals),
            )
            if weak is None:
                continue
