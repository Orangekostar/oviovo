...
class RuntimeVisModule:
    ...
    def _all_anchors_unique(raw_proposals: List[Proposal2D]) -> bool:
        ...
        for p in raw_proposals:
            ...
            if not anchor:
                return False  # unlabeled proposals might merge with each other
    ...
    def _score_pair(
        ...
        # --- Early-reject: cheap label/identity checks BEFORE geometry ---
        # In anchor_first_sam mode, 98% of pairs are rejected by label mismatch.
        # Computing adjacency, depth, and plane scores for these pairs is wasted work.
        semantic_blocked = self._semantic_blocked_residual_merge(feature_a.proposal, feature_b.proposal)
        if semantic_blocked:
        ...
        missing_label = self._missing_required_anchor_label(feature_a.proposal, feature_b.proposal)
        if missing_label:
            return RuntimeMergeDecision(
                mask_id_a=feature_a.proposal_id, mask_id_b=feature_b.proposal_id,
                adjacency_score=0.0, depth_continuity_score=0.0, plane_similarity_score=0.0,
        ...
        label_mismatch = self._required_anchor_labels_mismatch(feature_a.proposal, feature_b.proposal)
        if label_mismatch:
            return RuntimeMergeDecision(
                mask_id_a=feature_a.proposal_id, mask_id_b=feature_b.proposal_id,
                adjacency_score=0.0, depth_continuity_score=0.0, plane_similarity_score=0.0,
        ...
        identity_mismatch = self._required_anchor_identity_mismatch(feature_a.proposal, feature_b.proposal)
        ...
        # --- Only now: expensive geometry computation ---
        ...
        rejected_due_to_background_conflict = False
        if cross_class_anchor_conflict:
            accepted = False
        ...
        breakdown: Dict[str, float | int | bool | str] = {
            ...
            "missing_required_anchor_label": False,   # already returned early above
            "required_anchor_labels_mismatch": False,  # already returned early above
            "required_anchor_identity_mismatch": False, # already returned early above
            "semantic_blocked_residual_merge": False,   # already returned early above
